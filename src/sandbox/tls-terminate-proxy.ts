...
async function forwardUpstream(
  ...
  if (sigv4Plan?.action === 'resign') {
    ...
    if (payloadHash === undefined) {
      ...
      // endpoints reject chunked uploads without a Content-Length). A
      // bodyless re-sign keeps its original shape — no proxy-injected
      // `Content-Length: 0` for strict upstreams to trip on.
